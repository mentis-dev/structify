    ...
    async def validate_stakeholders(
        ...
        for group_idx, group in enumerate(duplicate_groups):
            try:
                ...
                prompt_file_path = os.path.join(os.path.dirname(__file__), "..", "prompts", "validator_duplicate_prompt.txt")
                try:
                    with open(prompt_file_path, "r", encoding="utf-8") as f:
                        prompt_template = f.read()
                    prompt = prompt_template.format(group_str=group_str)
        ...
        for stakeholder in validation_sample:
            try:
                ...
                prompt_file_path = os.path.join(os.path.dirname(__file__), "..", "prompts", "validator_category_prompt.txt")
                try:
                    with open(prompt_file_path, "r", encoding="utf-8") as f:
                        prompt_template = f.read()
                    prompt = prompt_template.format(name=name, category=category, role=role)
                except FileNotFoundError:
                    raise FileNotFoundError(f"Prompt file not found at: {prompt_file_path}")

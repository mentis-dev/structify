...
# Import our FAISS-based retrieval components
...
def initialize_raptor(index_name="documents_index", embedding_dimension=3072):
    """Initialize the FAISS-based Raptor Retriever"""
    # Initialize VectorStoreManager with FAISS
    ...
    # Initialize Raptor retriever with the manager
    raptor_retriever = RaptorRetriever(
        vectorstore_manager=manager,
        tree_depth=3,
        similarity_top_k=5,  # Adjust based on your needs
...
# Function to process documents with Raptor
async def process_documents_with_raptor(documents, namespace, fresh_start=False):
    """Process documents using Raptor and return retrieved context"""
    # Initialize the raptor retriever
    raptor = initialize_raptor()

    # Insert documents into Raptor (this builds the hierarchical index)
    print(f"Inserting {len(documents)} documents into Raptor index...")
    await raptor.insert(documents, namespace=namespace, fresh_start=fresh_start)

    # Ensure indexes are saved
    raptor.persist()
...
# Function to retrieve relevant documents using Raptor
def retrieve_context(raptor, query_text, namespace, mode=QueryModes.collapsed):
    """Retrieve relevant documents using Raptor retriever"""
...
async def main():
    ...
    parser.add_argument("--max-documents", type=int, default=None,
                        help="Maximum number of documents to process per brain (None for all)")
    parser.add_argument("--max-brains", type=int, default=None,
                        help="Maximum number of brains to process (None for all)")
    parser.add_argument("--model", default="openai/gpt-4o", help="The LLM model to use")
    parser.add_argument("--use-raptor", action="store_true", help="Use Raptor retriever for document processing")
    parser.add_argument("--raptor-query", default=None, help="Query to use with Raptor retriever")
    ...
    initial_state = InputState(
        ...
        use_raptor=args.use_raptor,
        raptor_query=args.raptor_query or "Identify stakeholders, factors, and pain points"
    )
    ...
    if args.use_raptor:
        print(f"Using Raptor retriever with query: {args.raptor_query or 'Default query'}")
    ...
    try:
        ...
        graph_config = {
            ...
            "raptor_initialized": False  # Flag to track if Raptor has been initialized
        }

        # If using Raptor, adapt the workflow to use it
        if args.use_raptor:
            # This needs to be implemented in your graph workflow
            # The idea is to load documents first, then use Raptor for retrieval
            pass

...
from core.causality_analyzer import analyze_pain_point_causality
...
# Session state for causality analysis
if "causality_results" not in st.session_state:
    st.session_state.causality_results = None

if "show_causality_results" not in st.session_state:
    st.session_state.show_causality_results = False
...
with tab5:
    ...
    if st.session_state.aggregated_results and "pain_points" in st.session_state.aggregated_results:
        pain_points = st.session_state.aggregated_results["pain_points"]
        
        if len(pain_points) >= 2:  # Need at least 2 pain points for causality
            st.divider()
            st.subheader("🔗 Pain Point Causality Analysis")
            
            col1, col2 = st.columns(2)
            
            with col1:
                if st.button("Analyze Causality Relationships"):
                    with st.spinner("Analyzing causal relationships between pain points..."):
                        try:
                            # Get text context based on extraction mode
                            text_context = ""
                            ...
                            if st.session_state.extraction_mode == "Selected Documents":
                                ...
                                if hasattr(st.session_state, "document_contents"):
                                    for doc_content in st.session_state.document_contents.values():
                                        if isinstance(doc_content, list):
                                            text_context += " ".join(doc_content[:5])  # Limit to first 5 chunks per doc
                                        else:
                                            text_context += doc_content
                            
                            elif st.session_state.extraction_mode == "Direct Text Input":
                                # Use text inputs as context
                                text_context = " ".join([t.get("text", "") for t in st.session_state.text_inputs])
                            
                            elif st.session_state.extraction_mode == "DOCX Directory":
                                # Use DOCX content as context
                                if hasattr(st.session_state, "docx_contents"):
                                    text_context = " ".join([content["content"] for content in st.session_state.docx_contents.values()])
                            
                            # Limit context length to avoid token limits
                            text_context = text_context[:5000] if text_context else ""
                            
                            # Run causality analysis
                            causality_results = async_to_sync(analyze_pain_point_causality(
                                extraction_results=st.session_state.aggregated_results,
                                text_context=text_context,
                                model=st.session_state.extraction_model,
                                output_dir="causality_analysis",
                                verbose=True
                            ))
                            
                            if not causality_results.get('error'):
                                st.session_state.causality_results = causality_results
                                st.success("✅ Causality analysis completed successfully!")
                                
                                # Show quick summary
                                relationships = causality_results.get('causal_relationships', [])
                                chains = causality_results.get('causal_chains', [])
                                
                                st.info(f"Found {len(relationships)} causal relationships and {len(chains)} causal chains")
                                
                            else:
                                st.error(f"❌ Causality analysis failed: {causality_results.get('error')}")
                                
                        except Exception as e:
                            st.error(f"Error during causality analysis: {str(e)}")
            
            with col2:
                if hasattr(st.session_state, 'causality_results') and st.session_state.causality_results:
                    if st.button("View Causality Results"):
                        st.session_state.show_causality_results = True
                        
                    if st.button("Download Causality Graph"):
                        try:
                            causality_json = json.dumps(st.session_state.causality_results, indent=2)
                            st.download_button(
                                label="Download Causality JSON",
                                data=causality_json,
                                file_name=f"causality_analysis_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json",
                                mime="application/json"
                            )
                        except Exception as e:
            ...
            st.info("🔗 Causality analysis requires at least 2 pain points")

    # Display causality results if available
    if (hasattr(st.session_state, 'causality_results') and 
        st.session_state.causality_results and 
        hasattr(st.session_state, 'show_causality_results') and 
        st.session_state.show_causality_results):
        
        st.divider()
        st.subheader("🕸️ Causality Analysis Results")
        
        causality_results = st.session_state.causality_results
        
        # Summary metrics
        relationships = causality_results.get('causal_relationships', [])
        chains = causality_results.get('causal_chains', [])
        graph_metrics = causality_results.get('graph_metrics', {})
        
        col1, col2, col3, col4 = st.columns(4)
        
        with col1:
            st.metric("Causal Relationships", len(relationships))
        with col2:
            st.metric("Causal Chains", len(chains))
        with col3:
            st.metric("Network Nodes", graph_metrics.get('total_nodes', 0))
        with col4:
            density = graph_metrics.get('density', 0)
            st.metric("Network Density", f"{density:.3f}")
        
        # Causality tabs
        caus_tab1, caus_tab2, caus_tab3, caus_tab4 = st.tabs([
        ...
        with caus_tab1:
            ...
            if relationships:
                ...
                for rel in relationships:
                    rel_data.append({
                        "ID": rel.get('id', ''),
                        "Cause": rel.get('cause_pain_point_id', ''),
                        "Effect": rel.get('effect_pain_point_id', ''),
                        "Type": rel.get('causality_type', ''),
                        "Strength": f"{rel.get('strength', 0)}%",
                        "Confidence": f"{rel.get('confidence', 0)}%",
                        "Time Delay": rel.get('time_delay', 'Unknown'),
                        "Mechanism": rel.get('mechanism', '')[:100] + "..." if len(rel.get('mechanism', '')) > 100 else rel.get('mechanism', '')
                    })
                ...
                for i, rel in enumerate(relationships):
                    with st.expander(f"🔗 {rel.get('id')}: {rel.get('cause_pain_point_id')} → {rel.get('effect_pain_point_id')}"):
                        st.write(f"**Type:** {rel.get('causality_type')}")
                        st.write(f"**Strength:** {rel.get('strength')}%")
                        st.write(f"**Confidence:** {rel.get('confidence')}%")
                        st.write(f"**Time Delay:** {rel.get('time_delay', 'Unknown')}")
                        st.write(f"**Mechanism:** {rel.get('mechanism', 'Not specified')}")
                        if rel.get('evidence'):
                            st.write(f"**Evidence:** {rel.get('evidence')}")
            else:
                st.info("No causal relationships found")
        
        with caus_tab2:
            st.subheader("Causal Chains")
            if chains:
                for i, chain in enumerate(chains):
                    with st.expander(f"🔗 Chain {i+1}: {' → '.join(chain.get('pain_point_sequence', []))}"):
                        st.write(f"**Chain ID:** {chain.get('chain_id')}")
                        st.write(f"**Sequence:** {' → '.join(chain.get('pain_point_sequence', []))}")
                        st.write(f"**Overall Strength:** {chain.get('overall_strength')}%")
                        st.write(f"**Description:** {chain.get('chain_description', 'Not specified')}")
            else:
                st.info("No causal chains identified")
        
        with caus_tab3:
            st.subheader("Root Causes & Ultimate Effects")
            
            col1, col2 = st.columns(2)
            
            with col1:
                st.write("**🌱 Root Causes** (No incoming causes)")
                root_causes = causality_results.get('root_causes', [])
                if root_causes:
            ...
            with col2:
                st.write("**🎯 Ultimate Effects** (No outgoing effects)")
                ultimate_effects = causality_results.get('ultimate_effects', [])
                if ultimate_effects:
                    for effect in ultimate_effects:
                        st.write(f"• {effect}")
                else:
                    st.info("No ultimate effects identified")
            
            # Central nodes
            st.write("**🎯 Most Central Pain Points**")
            central_nodes = causality_results.get('central_nodes', [])
            if central_nodes:
                for node, centrality in central_nodes:
                    st.write(f"• {node} (centrality: {centrality:.3f})")
            else:
                st.info("No central nodes identified")
        ...
        with caus_tab4:
            ...
            if graph_metrics:
                ...
                with col1:
                    ...
                    st.write(f"• Total nodes: {graph_metrics.get('total_nodes', 0)}")
                    st.write(f"• Total edges: {graph_metrics.get('total_edges', 0)}")
                    st.write(f"• Network density: {graph_metrics.get('density', 0):.3f}")
                
                with col2:
                    st.write("**Connectivity:**")
                    st.write(f"• Strongly connected components: {graph_metrics.get('strongly_connected_components', 0)}")
                    st.write(f"• Weakly connected components: {graph_metrics.get('weakly_connected_components', 0)}")
            
            # Node metrics table
            node_metrics = causality_results.get('node_metrics', {})
            if node_metrics:
                ...
                for node_id, metrics in node_metrics.items():
                    node_data.append({
                        "Pain Point": node_id,
                        "In-Degree": metrics.get('in_degree', 0),
                        "Out-Degree": metrics.get('out_degree', 0),
                        "Betweenness Centrality": f"{metrics.get('betweenness_centrality', 0):.3f}",
                        "PageRank": f"{metrics.get('pagerank', 0):.3f}"
                    })
